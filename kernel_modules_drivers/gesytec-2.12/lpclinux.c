 ...
 *  Nov 2007 - Release 2.10 updated to 2.6.23, added udev support
 *
...
#else
static irqreturn_t lpc_isa_interrupt( int irq, void *ptr)
#endif
    {
    DEV *dev;
...
#else
static irqreturn_t lpc_pci_interrupt( int irq, void *ptr)
#endif
    {
    DEV *dev = (DEV *)ptr;
    u32 cport;
...
#else
    if( request_irq(irq, lpc_isa_interrupt, IRQF_DISABLED|IRQF_SHARED, DRIVER_DEV_NAME, dev) )
#endif
        {
...
#else
        if( request_irq(dev->irq, lpc_pci_interrupt, IRQF_DISABLED|IRQF_SHARED, DRIVER_DEV_NAME, dev) )
#endif
            {
            err("unable to register IRQ%d\n", dev->irq);
#if 0
